...
class ESMChainAEmbedder:
    ...
    def embed_chain(self, chain_seqs: list) -> (torch.Tensor, torch.Tensor):
        ...
        with torch.no_grad():
            results = self.model(tokens, repr_layers=[36], need_head_weights=False)

        embeddings = results["representations"][36][:, 1:-1, :]          # remove [CLS], [EOS]
...
class ChainBAutoregressiveModel(nn.Module):
    ...
    def generate(
        ...
        generated = torch.full((B, 1), self.alphabet.cls_idx, dtype=torch.long, device=device)
        for _ in range(max_length):
            logits = self.forward(generated, chainA_emb)  # [B, L, vocab_size]
            next_token_logits = logits[:, -1, :]          # [B, vocab_size]
            next_token = torch.argmax(next_token_logits, dim=-1, keepdim=True)  # [B, 1]
            generated = torch.cat([generated, next_token], dim=1)
            # If all predicted <eos>=2, we can break early
            if (next_token == self.alphabet.eos_idx).all():
                break

        # Optionally remove the initial <cls> and final <eos> for a clean output
        # Or you can do so in a "detokenize" function.
        return generated
...
class CombinedDesign(nn.Module):
    ...
    def generate(self, chainA_seq: str, max_length=128) -> torch.Tensor:
        esm_emb, mask = self.esm_embedder.embed_chain(chainA_seq)
        md_emb = self.md_embedder(esm_emb, mask)
        fused_emb = esm_emb + self.md_proj(md_emb)
        
        generated_tokens = self.chainB_decoder.generate(fused_emb, max_length=max_length)
        # for _ in range(max_length):
    ...
    def detokenize(self, token_ids: torch.Tensor) -> str:
        ...
        tokens = token_ids.squeeze(0).tolist()
        s = []
        for t in tokens:
            # skip special tokens (0=<cls>,1=<pad>,2=<eos>,3=<unk>, etc.)
            if t < 4:
                continue
            s.append(self.alphabet.get_tok(t))
        return ''.join(s) 
